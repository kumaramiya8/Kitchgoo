...
const Delivery = () => {
  ...
  const allOrders = useMemo(() => {
    ...
    const posDelivMapped = (orders || [])
      .filter(o => o.orderType === 'delivery')
      .map(o => ({
        ...o,
        platform: o.deliveryChannel || 'In-House',
        source: 'pos',
        status: o.deliveryStatus || 'preparing',
        customer: o.customerName || 'POS Guest',
        phone: o.customerPhone || '',
        address: o.deliveryAddress || '',
        items: o.items ? o.items.reduce((s, i) => s + i.qty, 0) : 0,
      }));
    return [...delivMapped, ...mapped, ...posDelivMapped].sort((a, b) =>
      new Date(b.createdAt || 0) - new Date(a.createdAt || 0)
    );
  }, [deliveryOrders, onlineOrders, orders]);
  ...
  const handleAdvance = useCallback((order) => {
    ...
    if (order.source === 'online') {
      ...
      if (FLOW[order.status]) {
        updatePOSOrderDeliveryStatus(order.id, FLOW[order.status]);
      }
    } else {
      advanceDeliveryStatus(order.id);
    }
  }, [advanceDeliveryStatus, editOnlineOrder, updatePOSOrderDeliveryStatus]);

  const handleReject = useCallback((order) => {
    if (order.source === 'online') {
      editOnlineOrder(order.id, { status: 'cancelled' });
    } else if (order.source === 'pos') {
      updatePOSOrderDeliveryStatus(order.id, 'cancelled');
    } else {
      rejectDelivery(order.id);
    }
  }, [rejectDelivery, editOnlineOrder, updatePOSOrderDeliveryStatus]);
